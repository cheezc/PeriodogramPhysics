...
{
    target.draw(m_vertices, VERTEX_COUNT, sf::PrimitiveType::TriangleStrip, states);
}
...
void Box::centerTextHorizontally(sf::Text& text)
{
    ...
    text.setPosition(sf::Vector2f(x, Theme::borderSize + Theme::PADDING));
}
...
void Box::centerTextVertically(sf::Text& text)
{
    ...
    text.setPosition(sf::Vector2f(Theme::getBoxHeight() - Theme::PADDING, y));
}
