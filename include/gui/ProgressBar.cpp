    ...
    m_value(0.f)
{
    ...
    if (m_labelPlacement == LabelOutside)
    {
        if (orientation == Horizontal)
        {
            ...
            m_label.setPosition(sf::Vector2f(length + Theme::PADDING, Theme::PADDING));
...
void ProgressBar::setValue(float value)
{
    ...
    else
    {
        ...
        else if (m_labelPlacement == LabelOutside)
        {
            ...
            m_label.setPosition(sf::Vector2f(labelX, m_box.getSize().y + Theme::PADDING));
        }
    }
...
void ProgressBar::draw(sf::RenderTarget& target, const sf::RenderStates& states) const
{
    ...
    target.draw(m_bar, 4, sf::PrimitiveType::TriangleStrip, rs);
