...
void TextBox::setCursor(size_t index)
{
    if (index <= m_text.getString().getSize())
    {
        ...
        m_cursor.setPosition(sf::Vector2f(m_text.findCharacterPos(index).x, padding));
        ...
        if (m_cursor.getPosition().x > getSize().x - padding)
        {
            ...
            m_text.move(sf::Vector2f(-diff, 0));
            m_cursor.move(sf::Vector2f(-diff, 0));
        }
